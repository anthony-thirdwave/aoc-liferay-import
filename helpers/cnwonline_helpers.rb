...
def get_cnwonline_issue_files
  files = []
  Dir.glob("#{Dir.pwd}/assets/2016_test_issues/www.chicagocatholic.com/cnwonline/**/*").each do |file|
    if file.split("/")[-3] != "cnwonline" && file.split("/")[-2] != "cnwonline"
      if !file.include? "?"
        if !file.include? "images"
          if !file.include? "news"
...
def get_title(file, f)
  if file_id(file).include?("familyroom.aspx") || file_id(file) == "conversations.aspx"
...
def get_author(file, f)
  if file_id(file).include?("barron.aspx") || file_id(file) == "faithculture.aspx"
    author = "Bishop Robert Barron"
  elsif file_id(file).include?("familyroom.aspx")
...
def date_adjuster(date)
	return "" if date.empty?
	if date.length > 1
    da = (date[0].strip + ", 2016").gsub(" 2016,", "").gsub(",", "").split(" ")
    DateTime.new(da[2].to_i, Date::MONTHNAMES.index(da[0]), da[1].to_i)
  else
    da = date[0].gsub(",", "").split(" ")
    DateTime.new(da[2].to_i, Date::MONTHNAMES.index(da[0]), da[1].to_i)
  end
end
...
def get_file_date(file)
  ...
  [f.xpath('//p[@id="pubdate"]').children.to_s.strip.split("-")].each do |date|
    date_adjuster(date)
  end
end
...
def remove_ads_from_issue_images(images)
  new_images = []
  images.each do |image|
    new_images << image if !image.include? "ads"
  end
  new_images
end
